...
let tyck_defty ctx (dt: Nast.defty) =
  [ (dt.name, tyck_expr ctx { type_ = make_pi dt.args (Universe)
                            ; value = LitTy(dt.name)
                            })
  (* ; ("elim-" ^ dt.name, TODO) *)
  ]
...
let tyck_module m =
  let open Nast in
  { Tast.name = m.name
  ; Tast.defs = tyck_defs Builtins.builtins m.defs
  }
